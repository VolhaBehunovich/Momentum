...
async function getWeather() {
  ...
  const url = `https://api.openweathermap.org/data/2.5/weather?q=${inputCity.value}&lang=en&appid=b5b7dbc06c5b3f4538b61ed09cd90b1b&units=metric`;
...
function getQuote() {
  ...
  request.open("GET", "../js/quotes.json", true);
  request.send();
  ...
  request.onload = function () {
    ...
    if (request.status === 200) {
      quote.textContent = `"${jsObj.quotes[randomNum].quote}"`;
